...
void Ghast::Update(float elapsedTime){
...
void Ghast::Render(){
	...
	SGD::GraphicsManager::GetInstance()->DrawTextureSection(
		m_hImage,
		this->GetPosition(),
		{ ((float)(this->GetDirection()) * 96.0f) - 32.0f, 0.0f, ((float)(this->GetDirection()) * 96.0f), 32.0f },
		0.0f,
		{},
		SGD::Color(255, 255, 255),
...
SGD::Rectangle	Ghast::GetRect() const{

	return Enemies::GetRect();
}

void Ghast::HandleCollision(const IEntity* pOther){
