...
void Spire::Update(float elapsedTime){
...
void Spire::Render(){
	...
	SGD::GraphicsManager::GetInstance()->DrawTextureSection(
		m_hImage,
		this->GetPosition(),
		{ ((float)(this->GetDirection()) * 96.0f) - 32.0f, 96.0f, ((float)(this->GetDirection()) * 96.0f), 128.0f },
		0.0f,
		{},
		SGD::Color(255, 255, 255),
...
SGD::Rectangle Spire::GetRect() const{

	return Enemies::GetRect();
}

void Spire::HandleCollision(const IEntity* pOther){
