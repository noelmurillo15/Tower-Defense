...
void EntityManager::CheckCollisions(unsigned int bucket1, SGD::Rectangle rect)
{
	// Validate the iteration state
	assert(m_bIterating == false && "EntityManager::CheckCollisions - cannot collide while iterating");
	...
	for (unsigned int i = 0; i < vec1.size(); i++) {
		...
		// Check for collision between the entities
		if (rEntity1.IsIntersecting(rect) == true) {
			//	do something
			if (rect == Tower::GetInstance()->GetRect()){
				vec1[i]->HandleCollision(Tower::GetInstance());
			}
		}
	}
